            ...
            <Grid templateColumns='repeat(6, 1fr)' gap={8} py={8}>
                <GridItem colSpan={{base: 6, lg: 4}}>
                    <Form onSubmit={handleSubmit}>
                        <Accordion allowMultiple>
                            {components.map((item) => {
                                return (<AccordionItem key={item.id}>
                                    <h2>
                                        <AccordionButton _focus={{boxShadow: 'none'}} bg='purple.400'
                                                         _hover={{bg: 'purple.500'}} py={3} color='white'>
                                        ...
                                        <RadioGroup display='flex' flexWrap='wrap'
                                            ...
                                            {item.list.map((component) => {
                                                return (<Box key={component.id} mt={2} mr={4} w={"sm"}>
                                                    <Img w={60} objectFit='cover'
                                                         src={component.image_url}/>
                                                    <FormLabel htmlFor={component.name} mb={0}>
                                                        <Text as='span' fontWeight='medium'
                                                              letterSpacing='1px'>{component.name}</Text>
                                                    </FormLabel>
                                                    <Text>
                                                        ${component.price}
                                                    </Text>
                        ...
                        {!isLoggedIn && <Text w='xs'
                                              align='center'
                                              fontWeight='semibold' color='gray.500' fontSize='sm' mt={1}>Login
                            required.</Text>}
                        <Box my={3}>
                            {components.map((item) => {
                                return (<Box color='gray.500' mt={1} mb={1} fontSize='sm'
                                             fontWeight='semibold' key={item.type}>
